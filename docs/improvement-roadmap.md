...
- **Actions**：workflow 注入 `secrets.LLM_API_KEY` / `secrets.LLM_BASE_URL` + `vars.LLM_MODEL`；OpenAI 兼容模型可按供应商能力配置 `vars.LLM_JSON_MODE` / `vars.LLM_THINKING`（未配置时展开为空字符串且不发送对应参数）。
